...
def get_movies_for_julie_edges(start_year, end_year):
    """
    ...
    julie_movies = get_movies_for_julie()
    julie_edges = get_edges_for_movie_list(julie_movies, all_cast_edges)
    
    filtered_movies = filter_movies_by_year(julie_movies, start_year, end_year)
    ...
    export_edges_to_csv("julie_edges.csv", filtered_edges)
...
def create_julie_network():
    meryl_nodes_file_name = movielens_dir + 'julie-node.csv'
    meryl_edge_file_name = movielens_dir + 'julie-edges.csv'
    all_cast_edges = get_all_cast_edges()
...
def main():
    # ...
    juli_years = get_years_for_julie()
...
#         print("Number of edges in female actor graph:", len(female_actor_graph.edges))
    ...
    for year in sorted(map(int, julie_years)):
        print(get_years_for_julie)
        female_actor_graph = create_female_actor_graph(year, year, all_cast_edges, all_actor_map)
        ...
        meryl_deg_rank = next((i + 1 for i, (node, _) in enumerate(sorted_deg_centrality) if node == julie_node), None)
        meryl_close_rank = next((i + 1 for i, (node, _) in enumerate(sorted_close_centrality) if node == julie_node), None)
        ...
        meryl_eigenvector_rank = next((i + 1 for i, (node, _) in enumerate(sorted_eigenvector_centrality) if node ==julie_node), None)
